...
/* Cotele unui meci pentru detalii (?cotele=), 14 septembrie 2026: 1X2, sansa
   dubla, GG si peste/sub 1,5-2,5-3,5 pe tot meciul, la fiecare casa, cu cota de
   acum SI cea de deschidere. 590 KB -> cativa KB. */
...
/* Echipele oficiale (df_li_1_<id>): fara poze si adrese de profil. 11 KB -> ~4 KB. */
...
/* Golgheterii (tt_<turneu>_<etapa>): locul, jucatorul, echipa, golurile, pasele. 21 KB -> ~5 KB. */
...
export default {
  async fetch(request) {
    ...
    const cotele = ps.get('cotele') || '';
    if (cotele) {
      if (!/^[A-Za-z0-9]{4,16}$/.test(cotele)) return nu('id nepermis');
      const rc = await fetch(COTA(cotele), { headers: antet,
        cf: { cacheTtl: 300, cacheEverything: true } });
      const t = rc.ok ? taieCoteleMeciului(await rc.text()) : null;
      return new Response(t || '{}', { status: t ? 200 : 502, headers: { ...cors,
        'Content-Type': 'application/json', 'Cache-Control': 'public, max-age=' + (t ? 300 : 30) } });
    }

    const feed = ps.get('feed') || '';
    /* to_<turneu>_<etapa>_1 = clasamentul unei ligi, live (14 septembrie 2026) */
    const clasament = /^to_[A-Za-z0-9]{4,16}_[A-Za-z0-9]{4,16}_1$/.test(feed);
    /* tt_<turneu>_<etapa> = golgheterii; df_li_1_<id> = echipele oficiale (14 septembrie 2026) */
    const golgheteri = /^tt_[A-Za-z0-9]{4,16}_[A-Za-z0-9]{4,16}$/.test(feed);
    const formatia = /^df_li_1_[A-Za-z0-9]{4,16}$/.test(feed);
    /* r_1_1 = doar ce s-a schimbat acum (goluri, stare, ceas), proaspat - 14 septembrie 2026 */
    const schimbari = feed === 'r_1_1';
    if (!(/^f_1_-?\d+_\d+_[a-z]{2}_\d+$/.test(feed) ||
          /^df_(st|sui|sur)_1_[A-Za-z0-9]{4,16}$/.test(feed) ||
          clasament || golgheteri || formatia || schimbari)) return nu('feed nepermis');

    const taiat = ps.get('doar') === 'scor' && feed.startsWith('f_');
    ...
    if (taiat && r.status === 200) {
      /* AJ / AK = cartonasele rosii gazde / oaspeti (14 septembrie 2026) */
      const NEVOIE = ['AA', 'AB', 'AC', 'AO', 'AG', 'AH', 'AJ', 'AK'], bucati = [];
      for (const bloc of text.split('~')) {
        const m = {};
        for (const p of bloc.split('¬')) {
          const i = p.indexOf('÷');
